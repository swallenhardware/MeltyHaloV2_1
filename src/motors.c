...
int16_t motor1Setpoint = 0;
int16_t motor2Speed = 0;
int16_t motor2Setpoint = 0;
...
void runMotors() {
	...
	if(loopTime - lastSpeedChange > MOTOR_ACCEL_RATE) {
		...
		if(motor1Speed > motor1Setpoint) motor1Speed--;
		if(motor1Speed < motor1Setpoint) motor1Speed++;

		if(motor2Speed > motor2Setpoint) motor2Speed--;
		if(motor2Speed < motor2Setpoint) motor2Speed++;
	}
	...
	if(loopTime - lastMotorSendTime >= MIN_MOTOR_TIME) {
		...
		uint16_t speed1 = (uint16_t) ((motor1Speed > 0) ? motor1Speed : motor1Speed*-1);
		uint16_t speed2 = (uint16_t) ((motor2Speed > 0) ? motor2Speed : motor2Speed*-1);

#define DSHOT_RANGE 999U

		if(speed1 != 0) {//when speed == 0, we use command 0 instead of 1048. Command 0 arms and stops the motor
			//convert the speed value to be between 0 and 1000
			speed1 = ((uint32_t) speed1 * DSHOT_RANGE) / MOTOR_MAX_SPEED;
			if(speed1 > DSHOT_RANGE ) speed1 = DSHOT_RANGE;
		...
		issueDshotCommand(MOTOR1, speed1);

		if(speed2 != 0) {//when speed == 0, we use command 0 instead of 1048. Command 0 arms and stops the motor
			//convert the speed value to be between 0 and 1000
			speed2 = ((uint32_t) speed2 * DSHOT_RANGE) / MOTOR_MAX_SPEED;
			if(speed2 > DSHOT_RANGE ) speed2 = DSHOT_RANGE;
		...
		issueDshotCommand(MOTOR2, speed2);
	}
#endif

#ifdef MC_ONESHOT125//TODO: FIX THIS SECTION
	int16_t speed1 = ((uint32_t) speed * 2500U) / MOTOR_MAX_SPEED;
